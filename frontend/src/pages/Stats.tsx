...
import {
  TrendingUp, AlertTriangle, FileText, DollarSign,
} from 'lucide-react'
...
export default function Stats() {
  ...
  const severityData = stats ? Object.entries(stats.severity_distribution).map(([name, value]) => ({
    name,
    value,
    fill: SEV_COLORS[name as keyof typeof SEV_COLORS] || '#94a3b8',
  })) : []

  // Deviation trend (line chart, sorted by time)
  const deviationTrend = [...quotes]
    ...
    .map((q, i) => ({
      ...
      supplier: q.supplier_name.slice(0, 4),
      level: q.severity_level,
    }))

  // Severity by month (stacked bar) - group by severity per period
  const monthlyData = [...quotes]
    ...
    .reduce((acc, q) => {
      ...
      if (acc[month][q.severity_level as keyof typeof acc[typeof month]] !== undefined) {
        (acc[month] as any)[q.severity_level]++
      }
      return acc
  ...
  // Supplier anomaly rate (horizontal bar)
  ...
  const supplierChartData = Object.values(supplierData)
    .filter(s => s.total >= 1)
  ...
  return (
    <div className="h-full overflow-auto bg-gray-50 p-4 lg:p-6">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-500">AI Agent 运行数据统计与异常趋势分析</p>
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          <div className="bg-white rounded-2xl border p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500">总报价数</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500">平均偏离度</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500">待处理异常</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500">潜在节省</span>
            </div>
            <div className="text-2xl font-bold text-emerald-600">¥{(stats.total_potential_savings / 10000).toFixed(1)}万</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white rounded-2xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">异常级别分布</h3>
          <div className="h-52">
        ...
        <div className="bg-white rounded-2xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">月度异常趋势（堆叠）</h3>
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyChartData}>
                ...
                <Bar dataKey="正常" stackId="a" fill="#10b981" radius={[0, 0, 0, 0]} />
        ...
        <div className="bg-white rounded-2xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">偏离度趋势</h3>
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
